...
import { OCS_INTERNAL_CR_NAME, MINIMUM_NODES } from '../../../../constants/common';
...
export const CreateInternalCluster: React.FC<CreateInternalClusterProps> = ({ match }) => {
  ...
  const hasEnabledNextStep = !!(state.nodes.length >= MINIMUM_NODES && scName);

  const steps = [
    {
      name: 'Select capacity and nodes',
      id: 'step-1',
      component: <StepOne state={state} dispatch={dispatch} />,
      enableNext: hasEnabledNextStep,
    },
    {
      name: 'Configure',
      id: 'step-2',
      component: <StepTwo state={state} dispatch={dispatch} />,
    },
    {
      name: 'Review and create',
      id: 'step-3',
      component: <StepThree state={state} errorMessage={errorMessage} inProgress={inProgress} />,
      enableNext: hasEnabledNextStep,
