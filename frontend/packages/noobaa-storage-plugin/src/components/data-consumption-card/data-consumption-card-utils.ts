...
const iopsChartLegendData = (data: [BarChartData[], BarChartData[]]) => {
  ...
  const humanizedReads = humanizeDecimalBytesWithoutB(totalReads.toFixed(1));
  const humanizedWrites = humanizeDecimalBytesWithoutB(totalWrites.toFixed(1));
  return [
