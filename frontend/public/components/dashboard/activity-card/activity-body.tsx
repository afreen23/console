...
export const RecentEventsBody: React.FC<RecentEventsBodyProps> = ({ events, filter }) => {
  ...
  const onToggle = React.useCallback((uid: string) => {
    expanded.includes(uid) ? setExpanded(expanded.filter(e => e !== uid)) : setExpanded([...expanded, uid]);
  }, [expanded]);
  const isExpanded = React.useCallback((uid: string) => {
    return expanded.includes(uid);
  }, [expanded]);
  const eventItem = React.useCallback((props => (
    <EventItem isExpanded={isExpanded} onToggle={onToggle} {...props} />
  )), [isExpanded, onToggle]);
...
export const OngoingActivityBody: React.FC<OngoingActivityBodyProps> = ({
  ...
  const activitiesLoaded = loaded || (activities.length > 0 || prometheusActivities.length > 0);
  const allActivities = prometheusActivities.map(({ results ,loader }, idx) => (
    <Activity key={idx}>
      <AsyncComponent loader={loader} results={results} />
    </Activity>
  ));
  activities.sort((a, b) => +b.timestamp - +a.timestamp).forEach(({ resource, timestamp, loader }) =>
    allActivities.push(
    ...
    <>
      ...
      <div className="co-activity-card__ongoing-body">
        {!activitiesLoaded
          ...
          ) : activities.length || prometheusActivities.length
            ? allActivities
            : (
              <Activity>
