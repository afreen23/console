...
import { IRow, sortable, SortByDirection } from '@patternfly/react-table';
import { hasOCSTaint, hasTaints, getConvertedUnits } from '../../utils/install';
import { cephStorageLabel } from '../../selectors/selectors';
import { ZONE_LABEL } from '../../constants/common';
...
const getRows: GetRows = (
  ...
  const rows = filteredData.map((node: NodeKind) => {
    ...
    const cells: IRow['cells'] = [
      ...
      {
        title: node.metadata.labels?.[ZONE_LABEL] ?? '-',
      },
    ];
    return {
      cells,
